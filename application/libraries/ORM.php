...
class ORM_Core {
	...
	public function __get($column)
	{
		...
		elseif (isset($this->has_many[$column]))
		{
			 ...
			 if (isset($this->has_many[$column]['model']))
			{
				// Load the "end" model
				$model = ORM::factory(inflector::singular($this->has_many[$column]['model']));
			}
			else
			{
				// Load the "end" model
			...
			else
			{
				...
				if(is_array($this->has_many[$column]['foreign_key']))
				{
					$where = array();
					foreach($this->has_many[$column]['foreign_key'] as $fk)
					{
						$where["$fk"] = $this->object[$this->primary_key];
					}
					return $this->related[$column] = $model
						->orwhere($where)
						->find_all();
